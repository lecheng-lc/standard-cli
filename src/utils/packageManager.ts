...
import { hasGit, hasYarn, hasProjectYarn, hasPnpm3OrLater, hasPnpmVersionOrLater, hasProjectPnpm, hasProjectNpm } from './env.js'
...
const executeCommand = function executeCommand(command: string, args: string[], cwd: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    child.stderr?.on('close', (code: boolean) => {
      if (code) {
...
type binType = 'npm' | 'pnpm' | 'yarn'
class PackageManager {
  context: string = ''
  bin: binType = 'npm'
  needsPeerDepsFix: boolean = false
  ...
  async runCommand(command: string, args: string[]) {
    await executeCommand(this.bin, [...(PACKAGE_MANAGER_CONFIG[this.bin] as any)[command], ...(args || [])], this.context)
  }
}
